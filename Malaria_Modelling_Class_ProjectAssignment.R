...
#g <- graph_from_adjacency_matrix(adj_matrix, mode = "directed") #What does undirected mean? How does it differ from a directed graph?
g
##Add attributes
...
#SOLUTION
plot_network_betweeness <- function(g, data){ 
  ##Calculate degree centrality
...
plot_network_betweeness(g, data)
...
#SOLUTION
betweenness <- betweenness(g)
kenya_betweenness <- betweenness[data$Country == "Kenya"]
other_betweenness <- betweenness[data$Country != "Kenya"]

cat("b) betweenness Centrality Comparison:\n")
cat("   Kenya participants - Mean betweenness:", round(mean(kenya_betweenness), 2), "\n")
cat("   Other countries - Mean betweenness:", round(mean(other_betweenness), 2), "\n")
cat("   Kenya participants are", round(mean(kenya_degrees)/mean(other_betweenness), 1), "times more connected\n\n")
...
#SOLUTION
least_connected <- sort(degrees, decreasing = FALSE)[1:3]
cat("c) Least 3 most connected individuals:\n")
for(i in 1:3) {
...
top6_connected_betweenes<- sort(betweenness, decreasing = TRUE)[1:6]
top6_names_betweenes <- names(top6_connected)

cat("a) Top 6 most connected individuals based on betweenness:\n")
for(i in 1:6) {
  cat("   ", i, ".", top6_names_betweenes[i], "-", top6_connected_betweenes[i], "connections\n")
}
...
#SOLUTION 
##  Compare superspreader scenarios based on betweeness
# Scenario 1: Top 6 most connected as superspreaders
set.seed(42)
result_ss_connected_betweeness <- sis_model(adj_matrix, beta, gamma, initial_infected, days, superspreaders = top6_names_betweenes)

# Scenario 2: Random 6 people as superspreaders
set.seed(123)
random_ss_betweeness <- sample(data$Participant, 6)
set.seed(42)
result_ss_random_betweeness <- sis_model(adj_matrix, beta, gamma, initial_infected, days, superspreaders = random_ss_betweeness)

## d). Calculate superspreader effects
baseline_infections_btn <- sum(rowSums(result1$states))
#baseline_infections_btn
connected_ss_infections_btn <- sum(rowSums(result_ss_connected_betweeness$states))
#connected_ss_infections_btn
random_ss_infections_btn <- sum(rowSums(result_ss_random_betweeness$states))
#random_ss_infections_btn

connected_effect_btn <- round(((connected_ss_infections_btn - baseline_infections_btn) / baseline_infections_btn) * 100, 1)
#connected_effect_btn
random_effect_btn <- round(((random_ss_infections_btn - baseline_infections_btn) / baseline_infections_btn) * 100, 1)
#random_effect_btn

cat("\nb-d) Superspreader Effect Analysis Based on Betweeness:\n")
cat("   Baseline (no superspreaders):", baseline_infections_btn, "total infections\n")
cat("   Most connected as superspreaders:", connected_ss_infections_btn, "total infections (", connected_effect_btn, "% increase)\n")
cat("   Random people as superspreaders:", random_ss_infections_btn, "total infections (", random_effect_btn, "% increase)\n")
...
top3_indices <- which(rownames(adj_matrix) %in% names(top_connected_)[1:3])
...
##Exercise 3.2 - How many infections are prevented if we consider betweenness centrality for isolation? 
#SOLUTION 
##  Design targeted intervention
# Target the most connected individuals for intervention (e.g., isolation)
# Simple approach: remove top 3 most connected from network BASED ON betweeness
#top_connected
#Identify top 3 most connected individuals based on betweeness
top_connected_btn <- sort(betweenness, decreasing = TRUE)[1:3]
#top_connected_btn
cat("c) Top 3 most connected individuals:\n")
for(i in 1:3) {
  participant <- names(top_connected_btn)[i]
  connections <- top_connected_btn[i]
  country <- data$Country[data$Participant == participant]
  cat("   ", i, ".", participant, "(", country, ") -", connections, "connections\n")
}
adj_matrix_targeted_btn <- adj_matrix
top3_indices_btn <- which(rownames(adj_matrix) %in% names(top_connected_btn)[1:3])

# Zero out connections for top 3 (simulate isolation)
adj_matrix_targeted_btn[top3_indices_btn, ] <- 0
adj_matrix_targeted_btn[, top3_indices_btn] <- 0

set.seed(42)
result_targeted_btn <- sis_model(adj_matrix_targeted_btn, beta, gamma, initial_infected, days)
targeted_infections_btn <- sum(rowSums(result_targeted_btn$states))

cat("\ne) Targeted Intervention (Isolate top 3 connected):\n")
cat("   With targeted intervention:", targeted_infections_btn, "total infections\n")
cat("   Infections prevented:", baseline_infections_btn - targeted_infections_btn, "\n")
cat("   Effectiveness:", round(((baseline_infections_btn - targeted_infections_btn) / baseline_infections_btn) * 100, 1), "% reduction\n\n")
